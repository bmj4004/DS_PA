...
void draw_fov() {
    ...
    for (int i = 0; i < segments; i++) {
        for (int j = 0; j < segments; j++) {
            float h1 = (i / (float)segments - 0.5f) * h_fov_rad;
            float h2 = ((i + 1) / (float)segments - 0.5f) * h_fov_rad;
            float v1 = (j / (float)segments - 0.5f) * v_fov_rad;
            float v2 = ((j + 1) / (float)segments - 0.5f) * v_fov_rad;
            ...
            // 첫 번째 점
            float x2 = FOV_DISTANCE * cos(v1) * cos(h1);
            float y2 = FOV_DISTANCE * cos(v1) * sin(h1);
            float z2 = FOV_DISTANCE * sin(v1);
            
            // 두 번째 점
            float x3 = FOV_DISTANCE * cos(v1) * cos(h2);
            float y3 = FOV_DISTANCE * cos(v1) * sin(h2);
            float z3 = FOV_DISTANCE * sin(v1);
            
            // 세 번째 점
            float x4 = FOV_DISTANCE * cos(v2) * cos(h2);
            float y4 = FOV_DISTANCE * cos(v2) * sin(h2);
            float z4 = FOV_DISTANCE * sin(v2);
            
            // 네 번째 점
            float x5 = FOV_DISTANCE * cos(v2) * cos(h1);
            float y5 = FOV_DISTANCE * cos(v2) * sin(h1);
            float z5 = FOV_DISTANCE * sin(v2);
...
void display(void) {
    ...
    // FOV 시각화 그리기 - X축이 곡면의 중앙을 가리키도록 수정
    glPushMatrix();
    // 회전 제거 - FOV가 X축 방향을 따르도록
    draw_fov();
    glPopMatrix();
