...
void draw_heap_points(HeapNode* node, float max_distance) {
    ...
    // 카메라 위치로부터의 거리 계산 (미터 단위)
    float camera_distance = sqrt(
        node->x * node->x + 
        node->y * node->y + 
        (node->z - CAMERA_HEIGHT) * (node->z - CAMERA_HEIGHT)
    );
    
    // 1m 단위로 거리를 이산화
    int distance_step = (int)(camera_distance);
    
    const int num_colors = sizeof(colors) / sizeof(colors[0]);
    
    // 색상 인덱스 결정 (거리 단계에 따라)
    int color_index = (distance_step < num_colors) ? distance_step : (num_colors - 1);
...
int main(int argc, char** argv) {
    ...
    int point_count = 0;
    printf("grid size: %d x %d\n", global_grid->azimuth_cells, global_grid->elevation_cells);

    // FOV 내 점들만 순회
    for (int i = 0; i < global_grid->azimuth_cells; i++) {
        for (int j = 0; j < global_grid->elevation_cells; j++) {
            if (global_grid->cells[i][j].is_valid && 
                global_grid->cells[i][j].heap.root) {
                
                HeapNode* node = global_grid->cells[i][j].heap.root;
    ...
    printf("Range view에 그려진 점의 수: %d\n", point_count);

    // 출력 경로 설정 및 저장
    char* pcd_name = basename(argv[1]);
